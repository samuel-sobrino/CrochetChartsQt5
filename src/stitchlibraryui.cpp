    ...
    , ui(new Ui::StitchLibraryDialog)
{
    ...
    connect(ui->listView->horizontalHeader(), SIGNAL(sectionClicked(int)), SLOT(updateRowSizes()));
...
void
StitchLibraryUi::changeStitchSet(QString setName)
{
    ...
    const QSortFilterProxyModel* model
        = static_cast<const QSortFilterProxyModel*>(ui->listView->model());
...
void
StitchLibraryUi::setButtonStates(StitchSet* set)
{
...
StitchLibraryUi::resetLibrary()
{
    ...
    /*QPushButton* cancel =*/msgbox.addButton(tr("No, keep the library as is"),
    ...
    // FIXME: resetting a user set returns it to it's first save. keep user set's in a different
    // folder?
    ...
    if (ok && !text.isEmpty())
    {
        ...
        if (s)
        {
            QMessageBox::warning(this, tr("Stitch Exists"),
                                 tr("The stitch %1 is already in this stitch set").arg(text),
                                 QMessageBox::Ok);
    ...
    for (int i = set->stitchCount() - 1; i >= 0; --i)
    {
        ...
        if (set->stitchCount() == 1)
        {
            ...
            msgbox.setInformativeText(
                tr("If you wish to remove this stitch you must remove the set."));
            msgbox.setIcon(QMessageBox::Information);
            QPushButton* removeSet
                = msgbox.addButton(tr("Remove the stitch set too"), QMessageBox::AcceptRole);
            /*QPushButton* keepStitch =*/msgbox.addButton(tr("Keep this stitch"),
                                                          QMessageBox::RejectRole);
        ...
        if (!set->isMasterSet && StitchLibrary::inst()->masterHasStitch(s))
        {
            QMessageBox msgbox(this);
            msgbox.setText(
                tr("This stitch is linked to the master set. "
            ...
            msgbox.setIcon(QMessageBox::Question);
            QPushButton* confirm
                = msgbox.addButton(tr("Remove stitch from both lists"), QMessageBox::AcceptRole);
            /*QPushButton* cancel =*/msgbox.addButton(tr("Don't remove the stitch"),
                                                      QMessageBox::RejectRole);
...
StitchLibraryUi::addSelected()
{
    ...
    for (int i = 0; i < set->stitchCount(); ++i)
    {
        ...
        if (selected)
        {
            ...
            {
                QMessageBox msgbox;
                msgbox.setText(tr("A stitch with the name '%1' already exists in the master set.")
                                   .arg(s->name()));
                msgbox.setInformativeText(tr("Would you like to replace it with this one?"));
                msgbox.setIcon(QMessageBox::Question);
                QPushButton* confirm
                    = msgbox.addButton(tr("Replace the existing stitch"), QMessageBox::AcceptRole);
                QPushButton* no
                    = msgbox.addButton(tr("Keep original stitch as is"), QMessageBox::ApplyRole);
                QPushButton* cancel
                    = msgbox.addButton(tr("Stop adding stitches"), QMessageBox::DestructiveRole);
...
void
StitchLibraryUi::updateStitchSetProperties()
{
    const QSortFilterProxyModel* model
        = static_cast<const QSortFilterProxyModel*>(ui->listView->model());
    StitchSet* set = static_cast<StitchSet*>(model->sourceModel());
...
void
StitchLibraryUi::setupPropertiesBox()
{
    const QSortFilterProxyModel* model
        = static_cast<const QSortFilterProxyModel*>(ui->listView->model());
    StitchSet* set = static_cast<StitchSet*>(model->sourceModel());
...
void
StitchLibraryUi::hideProperties()
{
...
void
StitchLibraryUi::printStitchSet()
{
...
void
StitchLibraryUi::updateRowSizes()
{
    ui->listView->resizeRowsToContents();
}

void
StitchLibraryUi::filterStitchList(QString newText)
{
...
void
StitchLibraryUi::clearStitchFilter()
{
    ui->stitchFilter->clear();
}

void
StitchLibraryUi::createSet()
{
    bool ok;
    ...
    if (ok && !text.isEmpty())
    {
        ...
        if (found)
        {
            QMessageBox msgbox;
            msgbox.setText(
                tr("A stitch set with the name '%1' already exists in your library.").arg(text));
            msgbox.setInformativeText(tr("What would you like to do?"));
            msgbox.setIcon(QMessageBox::Question);
            QPushButton* overwrite
                = msgbox.addButton(tr("Replace the existing set"), QMessageBox::AcceptRole);
            QPushButton* rename
                = msgbox.addButton(tr("Change the name of the new set"), QMessageBox::ApplyRole);
            QPushButton* cancel
                = msgbox.addButton(tr("Don't add the new set"), QMessageBox::RejectRole);
        ...
        // WIN32: crashes if there isn't at least one stitch in the set when you add the model to
        // the view.
        set->createStitch("");
...
StitchLibraryUi::removeSet()
{
    ...
    msgbox.setIcon(QMessageBox::Question);
    QPushButton* remove
        = msgbox.addButton(tr("Yes, remove the set and it's files"), QMessageBox::AcceptRole);
...
StitchLibraryUi::exportSet()
{
    ...
    QString fileName = QFileDialog::getSaveFileName(this, tr("Save Stitch Set"), fileLoc,
                                                    tr("Stitch Set (*.set)"));
...
void
StitchLibraryUi::importSet()
{
    QString fileLoc = Settings::inst()->value("fileLocation").toString();
    QString fileName = QFileDialog::getOpenFileName(this, tr("Open Stitch Set"), fileLoc,
                                                    tr("Stitch Set (*.set)"));
...
void
StitchLibraryUi::updateSourceDropDown(QString setName)
{
    if (setName.isEmpty())
...
StitchLibraryUi::addStitchSet(QString fileName)
{
    ...
    if (test)
    {
        QMessageBox msgbox;
        msgbox.setText(
            tr("A stitch set with the name '%1' already exists in your library.").arg(set->name()));
        msgbox.setInformativeText(tr("What would you like to do?"));
        msgbox.setIcon(QMessageBox::Question);
        QPushButton* overwrite
            = msgbox.addButton(tr("Replace the existing set"), QMessageBox::AcceptRole);
        QPushButton* rename = msgbox.addButton(tr("Rename the new set"), QMessageBox::ApplyRole);
        /*QPushButton* cancel =*/msgbox.addButton(tr("Don't add the new set"),
                                                  QMessageBox::RejectRole);
